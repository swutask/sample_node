...
inputs = {
  ...
  log_group_name         = "${local.environment_vars.locals.project}-${local.environment_vars.locals.environment}-email-activity-consumer"
  app_name               = "${local.environment_vars.locals.project}-${local.environment_vars.locals.environment}-email-activity-consumer"
  ...
  command                = "node src/jobs/email-activity-consumer.js"
  memory                 = 128
}
