...
inputs = {
  ...
  log_group_name         = "${local.environment_vars.locals.project}-${local.environment_vars.locals.environment}-email-notification-consumer"
  app_name               = "${local.environment_vars.locals.project}-${local.environment_vars.locals.environment}-email-notification-consumer"
  ...
  command                = "node src/jobs/email-notification-consumer.js"
  memory                 = 128
}
